...
import { Mail, Lock, BookOpen, ArrowRight, User, Image as ImageIcon } from "lucide-react";
...
export default function Register() {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      if (error) {
        // Displays the actual backend error message if available
        toast.error(error.message || "Registration failed");
        return;
      }

      // Displays the missing success message
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center bg-[#FAF8F5] font-sans">
      <div className="w-full max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-12 min-h-[750px] lg:rounded-[2rem] lg:border lg:border-stone-200/60 lg:shadow-xl overflow-hidden bg-white">
        
        {/* Left Column: Brand Hero Section (Matches Login Exactly) */}
        ...
        <div className="col-span-1 lg:col-span-7 flex items-center justify-center p-8 sm:p-12 bg-[#FAF8F5] lg:bg-white">
          <div className="w-full max-w-md space-y-6">
            
            {/* Mobile/Global Header Block */}
            <div className="space-y-2">
              ...
              <p className="text-stone-500 text-sm font-medium">
                ...
                <Link href="/login" className="text-emerald-800 font-bold hover:underline underline-offset-4 transition-all">
                  Login here
                </Link>
              </p>
            </div>

            {/* Registration Form fields configured with HeroUI v3 compound elements */}
            <form className="space-y-4" onSubmit={handleRegister}>
              
              {/* FULL NAME FIELD */}
              <TextField name="name" type="text" className="w-full flex flex-col gap-1.5">
              ...
              <TextField name="image" type="url" className="w-full flex flex-col gap-1.5">
                <label htmlFor="image" className="text-sm font-semibold text-stone-700">
                  Profile Image URL
                </label>
                <InputGroup className="h-12 border border-stone-200 rounded-xl bg-white focus-within:ring-2 focus-within:ring-emerald-800 focus-within:border-emerald-800 transition-all shadow-sm">
                  <InputGroup.Prefix className="pl-3 flex items-center justify-center">
              ...
              {/* PASSWORD FIELD */}
              <TextField name="password" type="password" className="w-full flex flex-col gap-1.5">
              ...
              <Button
                type="submit"
                className="w-full h-12 text-sm font-bold bg-[#063725] text-white rounded-xl hover:bg-emerald-900 transition-all group mt-6 shadow-sm"
              >
                Register Account
            ...
            {/* Alternative Identity Google Button */}
            <Button
              variant="bordered"
