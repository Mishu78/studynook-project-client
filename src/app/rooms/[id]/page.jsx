...
const handleBookingSubmit = async (e) => {
  ...
  try {
    ...
    // ✅ CHANGE: Remove "/api" so it maps perfectly to your backend index.js path
    ...
    if (res.ok) {
      ...
      // Clean error catch for bad responses

      const errorData = await res.json().catch(() => ({}));

      toast.error(errorData.message || "Failed to secure booking.");
...
const handleUpdateSubmit = async (e) => {
    ...
    try {
      // 🎯 FIX 1: Fetch the authentic token context cleanly from Better Auth client instead of localStorage
      ...
      // 🎯 FIX 2: Explicitly parse numbers so they pass backend validation criteria smoothly
      ...
      const res = await fetch(`${baseUrl}/rooms/${id}`, {
        ...
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}` // Now passing the verified active token
        },
        body: JSON.stringify(updatedPayload)
      });

      if (res.ok) {
        toast.success("Study sanctuary configurations updated successfully!");
        onEditClose();
        fetchRoomDetails(); // Pull fresh data to update the UI instantly
      } else {
        const errorData = await res.json().catch(() => ({}));
        toast.error(errorData.message || "Failed to update target room parameters.");
      }
    } catch (err) {
      console.error(err);
      toast.error("Network runtime failure during update.");
    } finally {
      setActionLoading(false);
    }
  };
            ...
            <div className="flex gap-3 pt-4 border-t border-stone-100">
              ...
              {isOwner ? (
                ...
                <Button
                  ...
                  onClick={() => currentUser ? onBookOpen() : router.push('/login')}

                  className="w-full font-black rounded-xl bg-[#063725] text-white h-12 shadow-lg"
